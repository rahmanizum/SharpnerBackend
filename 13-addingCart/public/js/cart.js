...
function showOutput(response){
    ...
    response.data.forEach((ele,index) => {
        totalPrice+=Number(ele.pPrice)*Number(ele.noItem);
        ...
        <td class="d-flex ">
            ...
            <span class="mx-2">${ele.noItem}</span>
...
if(e.target && e.target.classList.contains("i-btn")){
    try{
        await axios.get(`http://192.168.29.221:7070/cart/add/${btnId}`);
        const response= await axios.get('http://192.168.29.221:7070/cart/productData');
        showOutput(response)
...
if(e.target && e.target.classList.contains("d-btn")){
    try{
        await axios.get(`http://192.168.29.221:7070/cart/delete/${btnId}`);
        const response= await axios.get('http://192.168.29.221:7070/cart/productData');
        showOutput(response)
...
if(e.target && e.target.classList.contains("r-btn")){
    try{
        await axios.get(`http://192.168.29.221:7070/cart/remove/${btnId}`);
        const response= await axios.get('http://192.168.29.221:7070/cart/productData');
        showOutput(response)
...
async function onCheckOut(e){
    e.preventDefault();
    try{
        const getData = await axios.get('http://192.168.29.221:7070/cart/productData');
...
// EVENT LISTENERS 
checkoutBtn.addEventListener('click',refresh);
