...
exports.getorderHistoryPage = async(request,response,next)=>{
try{
    response.sendFile('orderHistory.html', { root: 'views/user' });
...
exports.createOrder = async (request, response, next) => {
    try {
        ...
        if (cartProduct != "") {
            ...
            order.addProducts(cartProduct.map(ele => {
                ele.OrderItem = { quantity: ele.CartItem.quantity };
                return ele;
            }))
...
exports.getorderHistory = async(request,response,next)=>{
    try{
        ...
        //const pData=[];
        // for(ele of orders){
        //     const orderItems = await ele.getProducts();
        //     pData.push(orderItems);
        // }
